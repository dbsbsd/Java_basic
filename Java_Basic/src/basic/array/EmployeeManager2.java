   ...
   public static void main(String[] args) {
      ...
      while (true) {
         ...
         case 3:
            ...
            for (int i = 0; i <= count; i++) {
               if(userNum1.equals(userNums[i])){
                  // 사원의 정보: 사번, 이름, 나이, 부서명
                  System.out.printf("사번 : %s 이름 : %s 나이 : %d 부서명 : %s",userNums[i],names[i],ages[i],departments[i] );
               }else if(i == count){
         ...
         case 5:
            ...
            if(iDepartments==count) { // 일치하는 사번이 없는 경우
            ...
            if(YesDelete.equals("Y")) { // 대소문자 구분하지 않고 'Y'인 경우에만 삭제
                for(int j=iDepartments;j<count-1;j++) { 
